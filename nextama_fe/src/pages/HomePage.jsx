...
import BannerCarousel from '../components/ui/inhomewelcome/BannerCarousel';
...
const CountdownTimer = ({ targetDate }) => {
  ...
  Object.keys(timeLeft).forEach((interval) => {
    if (!timeLeft[interval] && interval !== 'seconds' && interval !== 'minutes' && interval !== 'hours') {
      return;
    }
  ...
  return timerComponents.length ? (
    <div className="flex items-center space-x-1 md:space-x-2">
      {timerComponents.reduce((acc, curr, idx) =>
        idx === 0 ? [curr] : [...acc, <span key={`sep-${idx}`} className="text-white text-sm md:text-md">:</span>, curr],
        []
      )}
    </div>
  ) : (
        ...
        >
          ...
          <div className="flex space-x-4 md:space-x-6 pb-4 w-max">
            {categories.map((category) => (
              ...
              >
                <div className={`p-3 md:p-4 rounded-full mb-3 md:mb-4 ${category.special ? 'bg-primary/20' : 'bg-primary/10'}`}>
                  <category.icon className={`h-8 w-8 md:h-10 md:w-10 ${category.special ? 'text-primary' : 'text-primary'}`} />
                </div>
                <span className={`text-sm md:text-base font-medium ${category.special ? 'text-primary' : 'text-foreground'} text-center leading-tight`}>
                  {category.name}
                </span>
              </Link>
            ))}
